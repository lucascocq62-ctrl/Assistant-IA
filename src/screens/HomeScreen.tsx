...
export function HomeScreen({ navigation }: Props) {
  ...
  const [showNoGoogleForm, setShowNoGoogleForm] = useState(false);
    ...
    <ScrollView style={styles.screen} contentContainerStyle={styles.container}>
      ...
      {canUseApp ? (
        ...
        <View style={styles.actionCard}>
          ...
          <Pressable accessibilityRole="button" style={styles.noGoogleButton} onPress={() => setShowNoGoogleForm((visible) => !visible)} disabled={isSigningIn}>
            <Text style={styles.noGoogleText}>{showNoGoogleForm ? 'Masquer la connexion sans Google' : 'Connexion sans Google'}</Text>
          </Pressable>
          {showNoGoogleForm ? (
            <View style={styles.noGoogleForm}>
              ...
              />
              <Pressable accessibilityRole="button" style={[styles.primaryButton, isSigningIn && styles.disabledButton]} onPress={handleNoGoogleSignIn} disabled={isSigningIn}>
                <Text style={styles.primaryText}>{isSigningIn ? 'Connexion…' : 'Valider sans Google'}</Text>
              </Pressable>
              <Text style={styles.formHint}>Si ce prénom et ce numéro existent déjà, Vet’Help réutilise le profil existant sans en recréer un, sans tenir compte des majuscules ni des espaces.</Text>
            </View>
          ) : null}
...
const styles = StyleSheet.create({
  ...
  noGoogleButton: { backgroundColor: '#f8fafc', borderColor: '#cbd5e1', borderWidth: 1, padding: 15, borderRadius: 14, alignItems: 'center' },
  noGoogleText: { color: '#0f172a', fontWeight: '900', fontSize: 15 },
